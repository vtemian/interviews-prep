...
class Solution:
    def clean_email(self, email: str) -> str:
        """
        An email address is composed by local address and domain. In the local address can occur . and +. Dots (.) need to
        be ignored and everything that is after + needs to be ignored.
        """

        local_address, domain = email.split('@')

        local_address = local_address.split('+')[0].replace('.', '')

        return f'{local_address}@{domain}'
