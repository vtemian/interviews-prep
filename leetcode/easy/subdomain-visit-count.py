...
class Solution:
    def subdomainVisits(self, cpdomains):
        ...
        for cp_domain in cpdomains:
            ...
            splited_roots = domain.split('.')

            for idx in range(len(splited_roots)):
                subdomain = '.'.join(splited_roots[idx:])

                if subdomain not in domains:
                    domains[subdomain] = int(visits)
                else:
                    domains[subdomain] += int(visits)

        return [
            '{} {}'.format(visits, subdomain)
            for subdomain, visits in domains.items()
        ]
